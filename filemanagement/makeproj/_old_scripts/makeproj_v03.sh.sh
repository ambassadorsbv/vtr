...
getinfo() {
  ...
  staatslotcheck=$(echo "$projectname" | grep STAATSLOTERIJ)
  ...
  if [[ -n $staatslotcheck ]]; then
    isstaatslot="1"
    projectname="${projectname/STAATSLOTERIJ-/}"
  fi
  ...
  elif [[ $isstaatslot == "1" ]]; then
    projectpath="/HEIMDALL/PROJECTS/staatsloterij/$projectfolder"
  else
